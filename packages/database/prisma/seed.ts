...
async function main() {
  await prisma.$transaction([
    ...
    prisma.playerGameplayDailyStat.deleteMany(),
  ...
  await prisma.cosmeticItem.createMany({
    data: [
      ...
      ...[
        ["sprout-avatar", "Sprout Scout", "AVATAR", 25, "sprout", "#173b2a", "#8df06d", 30],
        ["slime-avatar", "Nortix Slime", "AVATAR", 40, "slime", "#143e3a", "#42e8bd", 40],
        ["block-sunrise-banner", "Block Sunrise", "BANNER", 35, "sunrise", "#3b2347", "#ffb45f", 40],
        ["pixel-river-banner", "Pixel River", "BANNER", 50, "river", "#15344f", "#61d8ff", 50],
        ["pickaxe-badge", "Trail Miner", "BADGE", 25, "pickaxe", "#352b24", "#e6b86a", 50],
        ["pixel-heart-badge", "Good Heart", "BADGE", 45, "pixel-heart", "#421f32", "#ff719a", 60],
        ["block-builder-title", "Block Builder", "TITLE", 30, "block", "#2e3324", "#bad86b", 30],
        ["nether-voyager-title", "Nether Voyager", "TITLE", 60, "portal", "#431e1e", "#ff8159", 40],
        ["meadow-theme", "Meadow", "THEME", 50, "meadow", "#17382e", "#98e66e", 20],
        ["redstone-theme", "Redstone Lab", "THEME", 75, "redstone", "#2f2025", "#ff5b61", 30],
      ].map(([slug, name, type, price, icon, primary, accent, sortOrder]) => ({
        slug: String(slug),
        name: String(name),
        description: `${String(name)} is a permanent cosmetic unlocked with Sparks.`,
        type: type as CosmeticType,
        unlockMethod: CosmeticUnlockMethod.SPARKS,
        sparksPrice: Number(price),
        rarity: Number(price) >= 60 ? "RARE" : Number(price) >= 40 ? "UNCOMMON" : "COMMON",
        sortOrder: Number(sortOrder),
        preview: {
          primary: String(primary),
          accent: String(accent),
          icon: String(icon),
          pattern: type === "BANNER" ? "waves" : type === "THEME" ? "aurora" : "plain",
        },
      })),
    ],
  });
  ...
  await prisma.dailyQuest.createMany({
    data: [
      ...
      },
      {
        ...
        description: "Complete one Nortix campaign today. Backend verification is required.",
      ...
      },
      {
        ...
        description: "Vote for one eligible Nortix-verified server today.",
        type: "SERVER_VOTED",
        target: 1,
        sparksReward: 5,
        cadence: "DAILY",
      },
      {
        ...
        description: "Log in to a Nortix-verified server with its connected plugin running.",
